...
public class RootDetector {
    ...
    public List<String> checkDangerousBinaries() {
        List<String> found = new ArrayList<>();
        // Upstream (b42df36) concatenated HardcodedSignals.dangerousBinaries and binaryPaths and
        // stat()ed the result. That never worked: dangerousBinaries holds bare file names
        // ("su", "busybox"), and binaryPaths holds directories ("/system/bin/"), so the check
        // matched the directories every single time and never matched a binary.
        // Restored intent: cross product of directory x name.
        for (String dir : HardcodedSignals.binaryPaths) {
            if (!dir.endsWith("/")) dir = dir + "/";
            File dirFile = new File(dir);
            if (!dirFile.isDirectory()) continue;
            for (String name : HardcodedSignals.dangerousBinaries) {
                if (name.endsWith("/")) continue;
                File f = new File(dir + name);
                if (f.exists() && f.canRead()) found.add(f.getAbsolutePath());
            }
        }
        return found;
    }
